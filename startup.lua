...
local function controlTick(info, dt)
  ...
  if S.autoOut then
    ...
    -- === Failsafe por saturación baja ===
    local SAT_MIN = 50.0   -- no permitir que baje de 50%
    local SAT_REC = 60.0   -- volver al control normal al 60%

    if info.satP <= SAT_MIN then
      S.setOut = CFG.OUT_MIN
      S.action = S.action .. " | Protegiendo SAT baja"
    elseif info.satP >= SAT_REC then
      -- normal
    end
...
local function drawBar(mon, x, y, w, pct, color)
  ...
  if mon.isColor and mon.isColor() then
    for i=0,w-1 do
      ...
      mon.setBackgroundColor(i < fill and (color or colors.green) or colors.gray)
      mon.write(" ")
    end
